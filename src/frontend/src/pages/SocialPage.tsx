...
export default function SocialPage({
  ...
  const { data: leaderboard } = useQuery<LeaderboardEntry[]>({
    queryKey: ["socialLeaderboard"],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getSocialLeaderboard();
    },
    enabled: !!actor && !isFetching,
  });

  const { data: rewards } = useQuery<RewardFull[]>({
    queryKey: ["socialRewards"],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getSocialRewards();
    },
    enabled: !!actor && !isFetching,
  });
  ...
  const { data: challengesRaw } = useGetChallengesByCategory("social");
        ...
        <div className="flex justify-between items-center mb-12">
          <div>
            ...
            <p className="text-lg text-gray-600">
              {hubTexts?.socialPageSubtitle ??
                "Top performers in social media and community engagement"}
            </p>
          </div>
        ...
        {!hasLeaderboard && !hasRewards && !hasChallenges && (
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-500 max-w-md mx-auto">
              The social leaderboard, rewards, and challenges will appear here

...
export class StorageClient {
  ...
  public async getDirectURL(path: string): Promise<string> {
    ...
    let rawRef: unknown;
    try {
      rawRef = await this.actor.getFileReference(path);
    } catch (err) {
      console.error(
        `StorageClient.getDirectURL: backend error for path '${path}':`,
        err,
      );
      // Fallback: construct direct /file/ URL so callers that still use
      // useFileUrl() can at least attempt to render via the IC gateway.
      const fallbackUrl = `${window.location.origin}/file/${path}`;
      console.warn(
        `StorageClient.getDirectURL: falling back to direct URL: ${fallbackUrl}`,
      );
      return fallbackUrl;
    }

    // Handle both direct FileReference and Option type (?FileReference = [] | [FileReference])
    let fileReference: FileReference | null = null;
    if (rawRef === null || rawRef === undefined) {
      console.warn(
        `StorageClient.getDirectURL: no file reference found for path '${path}'`,
      );
      return "";
    }
    if (Array.isArray(rawRef)) {
      // Motoko Option type: [] = null, [ref] = Some(ref)
      if (rawRef.length === 0) {
        console.warn(
          `StorageClient.getDirectURL: file not found for path '${path}'`,
        );
        return "";
      }
      fileReference = rawRef[0] as FileReference;
    } else {
      fileReference = rawRef as FileReference;
    }

    if (!fileReference || !fileReference.hash) {
      console.warn(
        `StorageClient.getDirectURL: invalid file reference for path '${path}'`,
      );
      return "";
    }
    ...
    try {
      ...
      console.warn(
        `StorageClient.getDirectURL: invalid hash for path '${path}':`,
        err,
      );
      return "";
    }
